...
/// Loads the main settings file and resolves the configured `dynamic_configurations_file` path.
/// Errors if the field is not configured.
pub async fn resolve_dynamic_configurations_file_path() -> Result<String, String> {
    let settings_model = SettingsModel::load_async(None).await?;

    let dynamic_file = settings_model.dynamic_configurations_file.ok_or_else(|| {
        "dynamic_configurations_file is not configured in the main settings".to_string()
    })?;
