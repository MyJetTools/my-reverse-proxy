...
use tokio::net::{TcpSocket, TcpStream, UnixSocket};
...
pub async fn connect_to_http_localhost_endpoint(
    host: &str,
) -> Result<SendRequest<Full<Bytes>>, HttpClientError> {
    let connect_result = TcpStream::connect(host).await;
    //let connect_result = unix_socket.connect(unix_socket_path).await;
        ...
        Ok(tcp_stream) => {
            ...
            match handshake_result {
                Ok((mut sender, conn)) => {
                    ...
                    tokio::task::spawn(async move {
                        if let Err(err) = conn.with_upgrades().await {
                            println!("Http Connection to {} is failed: {:?}", host, err);
                        }
