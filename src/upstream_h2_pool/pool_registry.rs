...
use super::{ConnectorFactory, H2Pool, PoolDesc, PoolParams};
    ...
    TConnector: MyHttpClientConnector<TStream> + Send + Sync + 'static,
{
    ...
    pools: ArcSwap<SortedVecOfArc<i64, H2Pool<TStream, TConnector>>>,
    ...
    TConnector: MyHttpClientConnector<TStream> + Send + Sync + 'static,
{
    ...
    pub fn ensure_pool(
        ...
        let pool = Arc::new(H2Pool::new(desc, params, factory));
        let mut new_vec: SortedVecOfArc<i64, H2Pool<TStream, TConnector>> =
            SortedVecOfArc::from_iterator(cur.iter().cloned());
        new_vec.insert_or_replace(pool.clone());
        self.pools.store(Arc::new(new_vec));
        APP_CTX.prometheus.set_h2_pool_size(&name, pool_size);
        pool
    }

    pub fn get(&self, location_id: i64) -> Option<Arc<H2Pool<TStream, TConnector>>> {
    ...
    pub fn drain_unused(&self, active: &AHashSet<i64>) {
        let _g = self.write_lock.lock();
        let cur = self.pools.load_full();
        let mut new_vec: SortedVecOfArc<i64, H2Pool<TStream, TConnector>> = SortedVecOfArc::new();
        for pool in cur.iter() {
            if active.contains(&pool.desc.location_id) {
                new_vec.insert_or_replace(pool.clone());
            } else {
                pool.shutdown.store(true, Ordering::Relaxed);
                APP_CTX.prometheus.reset_h2_pool(&pool.desc.name);
            }
        }
        self.pools.store(Arc::new(new_vec));
    }
